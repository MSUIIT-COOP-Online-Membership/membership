<tr x-data="{ modalIsOpen : false }">
    ...
    @if(getCrudConfig('Premember')->delete or getCrudConfig('Premember')->update)
        <td>
            ...
            @if(getCrudConfig('Premember')->update && hasPermission(getRouteName().'.premember.update', 0, 0, $premember))
                <a href="@route(getRouteName().'.premember.update', $premember->id)" class="btn text-primary mt-1">
                    <i class="icon-pencil"></i>
                </a>
            @endif

            @if(getCrudConfig('Premember')->delete && hasPermission(getRouteName().'.premember.delete', 0, 0, $premember))
